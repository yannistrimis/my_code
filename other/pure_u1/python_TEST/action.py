...
def single_update(action,ind,mu,lattice,beta,nx,nt,D_update,q_help) :
...
def update(action, lattice, beta, nx, nt, D_update, q_help, traj) :
    for i_traj in range(traj) :
        for ind in range(nx**3*nt) :
            for mu in range(4) :
                lattice, action, q_help = single_update(action, ind, mu, lattice, beta, nx, nt, D_update, q_help)
    return lattice, action, q_help